// В самом верху, после импортов React:
import { api } from '../api';
...
useEffect(() => {
  const initAuth = async () => {
    ...
    if (token) {
      try {
        ...
        setUser({ email: userData.email, name: userData.email.split('@')[0] });
  ...
  try {
    ...
    setEventError(err.message || 'Не удалось сохранить событие');
  } finally {
    setEventLoading(false);
  }
};
  ...
  try {
    ...
    // 2. Верификация (временно — вручную)
    alert(`Код подтверждения отправлен на ${registerData.email}. Посмотри в терминал бэка.`);
    const code = prompt('Введите 6-значный код из консоли бэка:');
    if (!code) throw new Error('Отмена');

    await api.verify(registerData.email, code);

    // 3. Логин
    const { access_token } = await api.login(registerData.email, registerData.password);
    ...
    setUser({ email: userData.email, name: userData.email.split('@')[0] });
    setIsAuthenticated(true);
    localStorage.setItem('scheduleUser', JSON.stringify({ email: userData.email, name: userData.email.split('@')[0] }));
